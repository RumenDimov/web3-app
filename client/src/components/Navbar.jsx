...
const Navbar = () => {
  ...
  return (
    <div className='flex md:flex-row flex-col-reverse justify-between mb-[35px] gap-6'>
      ...
      <div className='lg:flex-1 flex flex-row justify-between max-w-[458px] py-2 pl-4 pr-2 h-[52px]
      bg-[#1c1c24] rounded-[100px]'>

        <input type='text' placeholder='Search for campaigns' className='flex w-ful font-epilogue
            ...
            ${!toggleDrawer ? '-translate-y-[100vh]' : 'translate-y-0'} transition-all duration-700`}>
              ...
              <ul className='mb-4'>
                {navlinks.map((link) => (
                  ...
                  <li key={link.name}
                  className={`flex p-4 ${isActive === link.name && 'bg-[#3A3A431]'}`}
                  onClick={() => {
                    ...
                    <img src={link.imgUrl} alt={link.imgUrl} className={`w-[24px] h-[24px]
                    object-contain ${isActive === link.name ? 'grayscale-0' : 'grayscale'}`}/>
                    <p className={`ml-[20px] font-epilogue font-semibold text-14px
                    ${isActive === link.name ? 'text-[#1DC071]' : 'text-[#808191]'} uppercase`}>{link.name}</p>
                  </li>
                ))}
              </ul>
